...
// as working and aimed at a stale port. One test on the join.
func TestQuicAddrFromReflectorAnswer(t *testing.T) {
	a, ok := quicAddr("182.6.166.95:35722")
	if !ok {
		t.Fatal("a public address from a reflector must be usable")
	}
	if got := a.String(); got != "/ip4/182.6.166.95/udp/35722/quic-v1" {
		t.Errorf("got %s", got)
	}
